		...
		<!--<div class="col-md-10">-->
			<div class="card card-primary" style="width:100%;">		
				<div class="card-header with-border">
					<h3 class="card-title" style="color:gray"><i class="fa fa-edit" data-card-widget="collapse"></i> Campaign</h3> 
				</div><!-- /.card-header -->	
				<div class="card-body" style="width:100%; display: none; height: auto;" align="justify">
					<!--<div class="container" align="justify">-->
						...
						<div class="row">
							<div class="col-md-12 col-lg-6">
								<div class="form-group">
									<label>Title *</label>
									<input type="text" class="form-control form-control-sm" width="100%" id="title" name="title" required placeholder="Title Campaign *" />
									<span class="help-block" style='color:red;'></span>
								</div>
							</div>
							<div class="col-md-12 col-lg-6">
								<div class="form-group">
									...
									<textarea id="description" name="description" class="form-control form-control-sm" placeholder="Deskripsi Campaign" rows="2" maxlength="800" style="resize: none;"></textarea>
									<span class="help-block" style='color:red;'></span>
								</div>
							</div>
						</div>
						<div class="row">
							...
							<div class="col-md-12 col-lg-6 date">
								<div class="form-group">
						...
						<div class="row">
							<div class="col-md-12 col-lg-6">
								<div class="form-group">
									<label>Template Message *</label>&nbsp;<a data-toggle="tooltip" title="note : Mohon parameter diawali dengan tanda # (hashtag) dan disesuaikan dengan nama-nama kolom yang ada di file excel yang akan diupload."><i class="fa fa-question-circle" aria-hidden="true"></i></a>
									<textarea id="form_message" name="form_message" class="form-control form-control-sm" placeholder="contoh : Kepada #name, tagihan anda sebesar #amount rupiah, sudah terlambat selama #overdue hari. Harap segera anda bayarkan. Abaikan jika sudah anda bayarkan. Terima kasih." required rows="3" maxlength="8000" style="resize: none;"></textarea>
									<p class="help-block" style="color:red; font-size: 9pt;">Contoh file excel silahkan klik <a href="http://192.168.100.110/app-voice_blast2/assets/uploads/trx_product_d.xlsx" target="_blank">disini</a>. Contoh template silahkan klik <a href="http://192.168.100.110/app-voice_blast2/assets/uploads/cth_template.txt" target="_blank">disini</a>.</p>
									<span class="help-block" style="color:red;"></span>
								</div>
							</div>
							<div class="col-md-12 col-lg-6">
								<div class="form-group">
									<label>Call Retry *</label>
									<select name="call_retry" id="call_retry" class="form-control form-control-sm" required placeholder="Pilih Salah satu *">
										<option value="">Select One...</option>
					...
					<!--</div>-->
					<hr>
					<h3 class="card-title" style="color:gray"><i class="fa fa-clock"></i> Scheduler & Upload Batch File</h3>
					<br />
					<!--<div class="container" align="justify">-->
						<div class="row">
							<table id="myTable" class="table table-bordered" role="grid" style="width: 100%; font-size:8pt;">
								<thead>
									<tr>
										...
										<th width="15%"><center>Upload File * (max. 2 MB)</center></th>
										<th width="5%"><center></center></th>
									</tr>
								</thead>
								<tbody id="isi">
			...
			<div class="card card-danger" style="width:100%;">
				<div class="card-header with-border">
					<h3 class="card-title" style="color:gray"><i class="fa fa-th-list" data-card-widget="collapse"></i> List of Campaign</h3>
				</div>
				<div class="card-body" style="height: auto;" align="justify">			
					<div class="table table-bordered table-hover" style="width:100%;background:#eeeeee;overflow-y: scroll;">
						<table class="cell-border compact hover" id="Show-Tables" width="100%" style="width:100%;font-size:10pt;word-break: break-all;">
							<thead align="center">
								<tr>
									...
									<th style="width: 15%;"><center>Product API</center></th>
									<th style="width: 15%;"><center>Title</center></th> 
									<th style="width: 25%;"><center>Template Message</center></th> 
							...
							<table style="width:100%; font-size:10pt;" border="0">
								...
								<tr style="line-height: 1.0;">
									...
									<td align="left" style="width:77%;"> 
										<select name="pname2" id="pname2" class="form-control form-control-sm" required placeholder="Pilih Product *">
											<option value="">Select One...</option>
											<?php foreach($product->result() as $rowproduct):?>
												<option value="<?php echo $rowproduct->id;?>"><?php echo $rowproduct->title;?></option>
											<?php endforeach;?>
										</select>
										<!--<input type="text" class="form-control form-control-sm" name="pname2" required />-->
									</td>
								</tr>
								<tr style="line-height: 1.0;">
									<td align="left" style="width:20%;"> Title </td>
...
$(document).ready(function()
{
	...
	$("#addrow").on("click", function () 
	{
		...
		cols += '<td width="5%"><center><div class="border-checkbox-group border-checkbox-group-default"><input class="border-checkbox" type="checkbox" id="checkbox" name="checkbox[]" /></div></center></td>';

		cols += '<td width="15%"><input type="text" data-date-format="YYYY-MM-DD" style="font-size:10pt;" id="start_date'+i+'" name="start_date['+i+']" class="form-control form-control-sm datepicker1" required placeholder="(yyyy-mm-dd)" /><span class="help-block" style="color:red;"></span></td>';

		cols += '<td width="15%"><input type="text" data-date-format="YYYY-MM-DD" style="font-size:10pt;" id="end_date'+i+'" name="end_date['+i+']" class="form-control form-control-sm datepicker2" required placeholder="(yyyy-mm-dd)" /><span class="help-block" style="color:red;"></span></td>';

		cols += '<td width="15%"><input type="text" style="font-size:10pt;" id="starttime'+i+'" name="starttime['+i+']" class="form-control form-control-sm timepicker" required placeholder="(HH:mm)" /><span class="help-block" style="color:red;"></span></td>';

		cols += '<td width="15%"><input type="text" style="font-size:10pt;" id="endtime'+i+'" name="endtime['+i+']" class="form-control form-control-sm timepicker" required placeholder="(HH:mm)" /><span class="help-block" style="color:red;"></span></td>';
		
		cols += '<td width="15%"><select id="status1'+i+'" name="status1['+i+']" class="form-control form-control-sm" required placeholder="Actived / Inactived ?"><option value="">Select One...</option><option value="1">Active</option><option value="0">Inactive</option></select><span class="help-block" style="color:red;"></span></td>';
		
		cols += '<td width="15%"><input type="file" id="userfile'+i+'" name="userfile['+i+']" class="form-control-sm" required accept=".xls, .xlsx" placeholder="Jenis Dokumen .xls atau .xlsx max. 2 MB " /><span class="help-block" style="color:red;"></span></td>';
		...
		cols += '<td width="5%"><center><button id="ibtnDel" class="btn btn-danger btn-outline-danger btn-sm1"><i class="fa fa-trash"></i> </button></center></td>';
		newRow.append(cols);
	...
	dataTable =  $('#Show-Tables').DataTable( 
	{
		...
		[
			...
			{ 
				"targets": 2, //third column - Product Name
				"width": "15%",
				"className": "text-center", //set alignment column to center
			},
			{ 
				"targets": 3, //fourth column - Title
				"width": "15%",
				"className": "text-center", //set alignment column to center
			},
			{ 
				"targets": 4, //fifth column - Format Message
				"width": "25%",
				"className": "text-center", //set alignment column to center
			},
			{ 
				"targets": 5, //fifth column - Status
				"width": "10%",
				"className": "text-center", //set alignment column to center
			},
			{ 
				"targets": 6, //fifth column - Action
				"width": "10%",
				"className": "text-center", //set alignment column to center
			}
		],
...
$(document).ready(function (e) {
	$('#upload').on('click', function () {
		...
		{ //jika checked lebih besar dari 0
			...
			form_Data.append('call_retry', call_retry);
...
function edit_trx(id)
{
	...
	$.ajax(
	{
		...
		success: function(data)
		{
			...
			$('[name="pname2"]').val(data.product_id);
...
function save()
{
		...
		e.preventDefault();
		$.ajaxFileUpload({
			url 			:"<?php echo site_url('Upl_Message/upload')?>", 
			secureuri		:false,
...
function save()
{
	...
	var myObj_h = {'company_id':company_id, 'prod_id':prod_id, 'title':title, 'form_message':form_message, 'startdate':startdate, 'description':description};
	//console.log(myObj_h);
	...
	{ //jika checked lebih besar dari 0
		...
		$('input[name="checkbox[]"]:checked').each(function(index,obj)
		{
			...
			formData.append('file', $('input[name="userfile['+i+']" ]').prop('files')[0]);
			formData.append('starttime',starttime);
			formData.append('endtime',endtime);
			formData.append('status1',status1);
			//var files = $("#upload-file")[0].files;
			//var userfile = $('input[name="userfile['+i+']" ]').val();
			i++;
			//var file = new FormData(userfile);
			myObj_d.push({'starttime':starttime,'endtime':endtime,'status1':status1,'userfile':formData});		
			//myObj_d.push({'starttime':starttime,'endtime':endtime,'status1':status1});		
		});
		...
		$.ajax(
		{
			...
			processData: false,
			type: "POST",
			data: {'id_d':myObj_d,'id_h':myObj_h,formData},
			...
			success: function(data)
			{
				...
				//if(data.status == false)
				//{ 
					//for (var c = 0; c < data.inputerror.length; c++) 
					//{
						//$('[name="'+data.inputerror[c]+'"]').parent().parent().addClass('has-error'); //select parent twice to select div form-group class and add has-error class
						//$('[name="'+data.inputerror[c]+'"]').next().text(data.error_string[c]); //select span help-block class set text error string
					//}
				//}
			},
			error: function (jqXHR, textStatus, errorThrown)
