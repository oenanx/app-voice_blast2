...
{	
	...
	private function _get_template($format, $upl)
	{
		//print_r($upl);
		//print_r($format);
		//exit();
	...
	function get_template($format, $upl)
	{
		//print_r($upl);
		//exit();
		//$this->_get_template($format, $upl);
		//if($_POST['length'] != -1)
		//$this->db->limit($_POST['length'], $_POST['start']);
		//$query = $this->db->get();
		for($i = 0; $i < count($upl); $i++)
		{
			...
			//$select1 = "CALL preview(".$i.", ".$phone.", '".$format."', '".$name."', ".$amount.", ".$overdue.");";
			//$query = $this->db->query($select1);
			//$this->db->select(''".$i."' as id, '".$phone."' as Phone, REPLACE(REPLACE(REPLACE('".$format."',\'#name\','".$nama."'),\'#amount\','".$amount."'),\'#overdue\','".$overdue."') as format_message;');
			//$query = $this->db->get();
		}
		
		//$user_stored_proc = "CALL preview(?, ?, ?, ?, ?, ?)";
		//$data = array('id' => $i, 'phone' => $phone, 'format' => $format, 'nama' => $name, 'amount' => $amount, 'overdue' => $overdue);
		//$query = $this->db->query($user_stored_proc, $data);
		return $query->result_array();
		//return $query->result_array();
		//return $query->row();
	}

	function count_template_filtered($format, $upl)
	{
		$this->_get_template($format, $upl);
		$query = $this->db->get();
		return $query->num_rows();
	...
	public function count_template_all($format, $upl)
	{
		//$this->db->from($this->table);
		//return $this->db->count_all_results();
		$this->_get_template($format, $upl);
	...
	{
	    $select1 = "SELECT a.id, a.product_id, c.product_name, a.company_id, b.company_name, a.title, a.format_message, a.description, a.start_date, a.end_date, a.status_id as status_id, CASE WHEN a.status_id = 0 THEN 'OPEN' WHEN a.status_id = 1 THEN 'START' WHEN a.status_id = 2 THEN 'COMPLETE' ELSE 'CANCELED' END as status_name FROM trx_product_h a, account_company b, product_master c WHERE a.company_id = b.id AND a.product_id = c.id AND a.id = ".$id.";";
	    $query1 = $this->db->query($select1);
		//return $query1->result_array();
		return $query1->row();
	}
	...
	{
	    $select3 = "SELECT a.id, c.product_name, a.batchid, a.start_date as startdate, a.end_date as enddate, start_datetime, end_datetime, a.active as factive, CASE WHEN a.active = 1 THEN 'ACTIVED' ELSE 'INACTIVED' END active, (SELECT COUNT(trx_schedule_id) FROM trx_product_d WHERE trx_schedule_id = a.id AND trx_product_h_id = b.id) jml_data FROM trx_schedule a, trx_product_h b, product_master c WHERE a.trx_product_h_id = b.id AND b.product_id = c.id AND b.id = ".$id.";";
 	...
 	public function Ins_trx_h($data)
	{
		...
		$sqlin = "INSERT INTO trx_product_h (company_id,product_id,title,format_message,description,start_date,end_date,status_id,create_by,create_date) VALUES (".$company_id.",".$product_id.",'".$title."','".$format_message."','".$description."','".$start_date."','".$end_date."',0,".$loginid.",NOW());";
	...
	public function Ins_sch($h_id,$data_sch)
	{
		...
		$sqlin2 = "INSERT INTO trx_schedule (trx_product_h_id,batchid,start_date,end_date,start_datetime,end_datetime,active,create_by,create_at) VALUES (".$trx_product_h_id.",UNIX_TIMESTAMP(NOW()),'".$data_sch['start_date']."','".$data_sch['end_date']."','".$data_sch['starttime']."','".$data_sch['endtime']."',".$data_sch['status1'].",".$loginid.",NOW());";
 	...
 	public function Ins_trx_d( $h_id,$sch_id,$upl)
	{
		...
		/*echo count($upl['starttime']);
		echo "<br/>";
		print_r($upl['starttime']);*/
		...
		for($i = 0; $i < count($upl['starttime']); $i++)
		{
			//id,trx_product_h_id,batchid,start_datetime,end_datetime,active,create_by,create_at,update_by,update_at
			$sqlin2 = "INSERT INTO trx_schedule (trx_product_h_id,batchid,start_datetime,end_datetime,active,create_by,create_at) VALUES (".$trx_product_h_id.",".$batchid.",'".$upl['starttime'][$i]."','".$upl['endtime'][$i]."',".$upl['status1'][$i].",".$loginid.",NOW());";
			$query2 = $this->db->query($sqlin2);
		}*/
		for($i = 0; $i < count($upl); $i++)
		{
			...
			$sqlin3 = "INSERT INTO trx_product_d (trx_product_h_id,trx_schedule_id,phone,name,overdue,amount) VALUES (".$h_id.",".$sch_id.",'".$phone."','".$name."',".$overdue.",".$amount.");";
