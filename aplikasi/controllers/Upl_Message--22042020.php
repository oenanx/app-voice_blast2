				...
				{ 
					...
					for($k=0; $k < count($this->upload->do_upload('wew')); $k++)
					{
						...
						foreach ($sheet as $row) 
						{
							...
							//$result[$j]['phone1']   = $row['E'];
							//$result[$j]['name1']    = $row['F'];
							//$result[$j]['overdue1'] = $row['G'];
							//$result[$j]['amount1']  = $row['H'];
							//$result[$j]['phone2']   = $row['I'];
							//$result[$j]['name2']    = $row['J'];

							$hasil[$j] = str_replace("#overdue",$result[$j]['overdue'],str_replace("#amount",$result[$j]['amount'],str_replace("#name",$result[$j]['name'],$format)));
							$id = $k + 1;
							//$row = array();
							$output[$j]['id'] = $id;
							$output[$j]['phone'] = $result[$j]['phone'];
							$output[$j]['template'] = $hasil[$j];
						...
						//$upl[$k] = $result;
						//print_r($upl[$k]);
						//$lists = $this->Mod_Upload->get_template($format, $upl[$k]);
						//print_r($data);
						//exit();
						/*
						foreach ($concat as $dtTemplate) 
						{
							//$no++;
							$rows = array();
							$rows[] = $dtTemplate->id;
							$rows[] = $dtTemplate->phone;
							$rows[] = $dtTemplate->template;
	        	...
	        	for($i=0; $i < $filelength; $i++)	
				{
					...
					{ 
                       	...
                       	for($k=0; $k < count($this->upload->do_upload('wew')); $k++)
						{
							...
							$list = $this->Mod_Upload->get_template();
							$datas = array();

							unlink(realpath('excel/'.$data_upload['file_name']));

							foreach ($list as $dtTemplate) 
							{
								//$no++;
								$rows = array();
								$rows[] = $dtTemplate->id;
							...
							$outputs = array(
											"draw" => $_POST['draw'],
											"recordsTotal" => $this->Mod_Upload->count_template(),
											"recordsFiltered" => $this->Mod_Upload->count_template(),
											"data" => $datas,
									);
